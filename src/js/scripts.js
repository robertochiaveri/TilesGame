var game = { //		main object
  ...
  config: { //		-	configurations
    ...
    toolbarButtons: [{
      ...
      handler: function() {
        game.fn.saveGame(game.config.labels.USERSAVE_LABEL);
      ...
      handler: function() {
        if (window.confirm("Load the saved game?")) {
          game.fn.loadGame(game.config.labels.USERSAVE_LABEL);
        };
        game.fn.animate({
  ...
  fn: { //		all METHODS container
        ...
        if (game.config.pushMultiple) {
          ...
          for (var i = 1; i <= tile.canPush.n; i++) {
            ...
            switch (tile.canPush.direction) {
              ...
              case game.config.labels.UP:
                tile.canPush.tiles.push(game.fn.getTile({
                  col: tile.col,
                  row: tile.row - i
                }));
                break;
    ...
    shuffle: function(n, refresh) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        movableTiles = game.fn.getMovableTiles({
          direction: d,
          pushing: false,
          moving: true,
          force: true
