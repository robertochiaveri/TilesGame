...
var game =	{										//		main object
	...
	config: {										//		-	configurations
		useTouch: useTouch(),						//		-	if the device supports it, listen for touch events instead of mouse events
		useKeyboard: true,							//		-	listen to keyboard events during game
		use3Dtransforms: use3D(),					//		-	if the device supports it, use 3D transforms (hardware accellerated) instead of 2D transforms
		useLstorage: useLocalStorage(),				//		-	if the devive supports it, use HTML5 localStorage to save games
		useCssAnimations: useCSS3Animations(),
		launchedAsApp: detectFullscreen(),			//		-	detects Safari fullscreen mode (for ex. launched as app from home screen)
		CSSprefixes: ["",getBrowserPrefix().css],		//		-	list of browser vendors prefixes to support for CSS3 properties
		easings: {
			EASING_TILES_MOVEMENT 	: 	"cubic-bezier(0.403, 0.543, 0.754, 1)",
		},
		size: {										//		-	size of the board (in tiles)
			h: 4,									//		-	-	tiles in a row (n. of columns)
			v: 4									//		-	-	tiles in a column (n. ofrows in the board)
		},
		marginH: 4,									//		-	minimum horizontal space btw the board and the screen, in %
		marginV: 4,									//		-	minimum vertical space btw the board and the screen, in %
		borderSize: 4,								//		-	thickness of the border around the board, in %
		pushMultiple: true,							//		-	when true tiles near the hole can be pushed by dragged tiles
		touchTolerance: .2,							//		-	percent of tile size to add during touch drag before dropping
		transitionDuration: {						//		-	costants for transition animations duration by type
			generic				:	150,			//		-	-	for tile movements on clicks, drags and gestures
			byKeyboard			:	100				//		-	-	for tile movement initiated by keyboard events
		},
		keycodes: {
			KEY_ARROW_LEFT		:	37,
			KEY_ARROW_UP		:	38,
		...
		toolbarButtons: [{	
					...
					handler:		function() { if(window.confirm("Start a new game?")) { game.fn.newGame(); }; }
				},{	
					id:				"save",
					description:	"Save the board",
					handler:		function() { 
										game.fn.saveGame(game.config.labels.USERSAVE_LABEL); addClass(document.getElementById("btn-save"),"on"); 										
									}
				},{	
					id:				"load",
					description:	"Load the saved board",
					handler:		function() { 
										if(window.confirm("Load the saved game?")) { game.fn.loadGame(game.config.labels.USERSAVE_LABEL); }; 
										game.fn.animate({element:document.getElementById("game"),animation:"load",duration:1000});				
									},
				},{	
					id:				"settings",
		...
		labels: {									//		-	constants for IDs, CLASSES and so on.
			...
			TILE_PREFIX:		"tile-",			//		-	-	id prefix for all tiles ids
			
			TBUTTON_PREFIX:		"btn-",			//		-	-	id prefix for all toolbar buttons ids

			SELECTED_LABEL:		"selected",			//		-	-	class added to a tile when is being dragged
			PRESSED_LABEL:		"pressed",			//		-	-	class added to a tile when is being pressed
			NO_TRANSITIONS:		"noTransitions",	//		-	-	class added to the game main div to inhibit transitions
			USETOUCH_LABEL:		"useTouch",			//		-	-	class added to the game main div when touch enabled			
			NO_TOUCH_LABEL:		"noTouch",			//		-	-	class added to the game main div when touch disabled			

			UP:					"up",				//		-	-	costant value for up
			RIGHT:				"right",			//		-	-	costant value for right
			DOWN:				"down",				//		-	-	costant value for down
			LEFT:				"left",				//		-	-	costant value for left
			
			LANDSCAPE:			"landscape",		//		-	-	costant value for landscape
			PORTRAIT:			"portrait",			//		-	-	costant value for portrait
			
			SAVEDGAMES_LABEL:	"savedGames",		//		-	-	string id for localStorage 
			AUTOSAVE_LABEL:		"automatic",		//		-	-	internal name for automatic savegames
			USERSAVE_LABEL:		"user",				//		-	-	internal name for user initiated savegames
	...
	fn:	{											//		all METHODS container
	
		init: function() {							//		-	entry point, starts up the game
					
			game.fn.createBoard();					//		-	-	creates the board, the tiles objects and the html				
			game.fn.metricsUpdate();
			game.fn.initListeners();				//		-	-	create event listeners enabling user interaction
			game.fn.refresh({transitions:false});	//		-	-	position each tile according to the game runtime			
			if (!game.fn.loadGame())
			{
				game.fn.newGame();					//		-	-	repeatedly changes random tiles 
			}
			else
			{
				game.fn.animate({element:document.getElementById("game"),animation:"load",duration:1000});				
				game.fn.start();					//		-	-	set the game as started. from this moment on the game checks if the user has won
			}	
		},
		start: function() {							//		-	(see above)
		...
		createBoard: function() {					//		-	creates the board, the tiles objects and the html		
			...
			for (var i=0; i<game.config.size.n;i++)	// repeat until (n) are created
			{
				...
				col++;								
				if (col >= game.config.size.h ) { row++; col = 0; };	
			};

			game.fn.saveGame(game.config.labels.SORTEDSAVE_LABEL)
		...
		metricsUpdate: function() {
			...
			if (typeof game.metrics.wrapperSize == "undefined") { game.metrics.wrapperSize = {}; }			
			...
			game.metrics.marginH = {
										percent: game.config.marginH,
										pixels : 0
			};

			// initialize the margin around the game board
			game.metrics.marginV = {
										percent: game.config.marginV,
										pixels : 0
			};			
			...
			var CSSstyleDeclaration =""
				+"padding:"	+Math.round(game.metrics.borderSize.pixels)+"px;"
				+"width:"	+game.metrics.width+"px; "
				+"height:"	+game.metrics.height+"px; "
				+"left:"	+game.metrics.left+"px; "
				+"top:"		+game.metrics.top+"px; ";
		...
		refresh: function(params) {
			...
			if (typeof params == "undefined") { params = { transitions: true, saveGame: false }; };
			...
			for (var i = 0; i<game.config.size.n; i++) {
				...
				for (var p = 0; p<game.config.CSSprefixes.length; p++)
				{
					...
					CSSstyleDeclaration += prefix+"transform:translate"+game.metrics.transforms3Dsupport[0]
					+"("
						+(tile.col*100)+"%,"
						+(tile.row*100)+"%"
						+game.metrics.transforms3Dsupport[1]
					+"); ";
				...
				if ( tile.correctCol != tile.col || tile.correctRow != tile.row ) {	win = false; }
			...
			if (params.saveGame === true) { game.fn.saveGame(); }		
		},
		initListeners: function() {
			...
			listenTo(document.getElementById(game.config.labels.BOARD_ID),(game.config.useTouch ? "touchstart" : "mousedown"),function(event) {
				...
				// ignore doubleclicks	
				if (event.timeStamp - game.runtime.latestSelection < 300)  { return false; }
				game.runtime.latestSelection = event.timeStamp;			
			
				game.fn.selectTile(game.fn.getTile(event),event);
			...
			listenTo(document.getElementById(game.config.labels.WRAPPER_ID),(game.config.useTouch ? "touchend" : "mouseup"),function(event) {
				...
				if (typeof game.runtime.selectedTile == "undefined" || game.runtime.selectedTile === null ) { return false; }
				...
				listenTo(document.body,"keyup",function(e) {
					...
					switch(e.which)
					{
						case game.config.keycodes.KEY_ARROW_LEFT: 
							game.fn.moveAnyTile({direction:game.config.labels.LEFT, pushing:e.ctrlKey})
						break;

						case game.config.keycodes.KEY_ARROW_UP: 
							game.fn.moveAnyTile({direction:game.config.labels.UP,pushing:e.ctrlKey})
						break;					

						case game.config.keycodes.KEY_ARROW_RIGHT: 
							game.fn.moveAnyTile({direction:game.config.labels.RIGHT,pushing:e.ctrlKey})
						break;					
						
						case game.config.keycodes.KEY_ARROW_DOWN: 
							game.fn.moveAnyTile({direction:game.config.labels.DOWN,pushing:e.ctrlKey})
						break;										
		...
		getTile: function (param) {
			...
			switch(typeof param) { 
				...
				case "object": 
					...
					if		(param.srcElement)	{ target = param.srcElement; }
					else if (param.pageX && param.pageY)	{ target = document.elementFromPoint(param.pageX,param.pageY) }
				...
				case "string":
					if (param.indexOf(prefix) === 0) { return tiles[param]; }
				return false;
				
				case "number":
					if (param < game.config.size.n) { return tiles[prefix+param]; }
				return false;
		...
		selectTile: function(tile,event) {
			...
			if (!game.runtime.running) { return false; }

			tile = game.fn.isTile(tile);
				...
				if (game.config.pushMultiple) {	
					...
					for (var i=1;i<=tile.canPush.n;i++) {
						...
						switch(tile.canPush.direction) {
							...
							case game.config.labels.UP:
								tile.canPush.tiles.push(game.fn.getTile({col:tile.col,row:tile.row-i}));
							break;
							
							case game.config.labels.RIGHT:
								tile.canPush.tiles.push(game.fn.getTile({col:tile.col+i,row:tile.row}));
							break;
		...
		deselectTile: function() {
			...
			var tile = game.fn.isTile(game.runtime.selectedTile); 
			if (!tile) { return false; };
			...
			if (typeof tile.newPosition == "undefined" ) { return false; };
		...
		canPush: function(tile) {
			...
			{	
				if (tile.col == hole.col || tile.row == hole.row)
				{
					...
					if (tilesToPush == 0) { return false; }
					
					if (tile.col == hole.col) // up or down
					{
						if (tilesToPush > 0) // down
						{ 
							return ({n:(Math.abs(tilesToPush)-1),direction:game.config.labels.DOWN});
						}
						else	// up
						{
							return ({n:(Math.abs(tilesToPush)-1),direction:game.config.labels.UP});
						}
					}
					else	// left or right
					{	
						...
						{ 
							return ({n:(Math.abs(tilesToPush)-1),direction:game.config.labels.RIGHT});
						}
						else	// left
						{
							return ({n:(Math.abs(tilesToPush)-1),direction:game.config.labels.LEFT});
						}
					}
				}
		...
		canMove : function(tile) {
			...
			if (holePosition == (tile.col)	+"-"+(tile.row-1))	{ return game.config.labels.UP;		}
			if (holePosition == (tile.col+1)+"-"+(tile.row))	{ return game.config.labels.RIGHT;	}
			if (holePosition == (tile.col)	+"-"+(tile.row+1))	{ return game.config.labels.DOWN;	}
			if (holePosition == (tile.col-1)+"-"+(tile.row))	{ return game.config.labels.LEFT;		}						
		
			return false;						
		},
		dragTile: function(event) {

			if (!game.runtime.running) { game.fn.deselectTile(); return false; }
			...
			if ( newPosY < tile.newPosition.top.min.pixels )	{ newPosY = tile.newPosition.top.min.pixels; 	};
			if ( newPosY > tile.newPosition.top.max.pixels )	{ newPosY = tile.newPosition.top.max.pixels; 	};
			if ( newPosX < tile.newPosition.left.min.pixels )	{ newPosX = tile.newPosition.left.min.pixels; };
			if ( newPosX > tile.newPosition.left.max.pixels )	{ newPosX = tile.newPosition.left.max.pixels; };					
			...
			switch(direction) {
				...
				case game.config.labels.RIGHT:	
					...
					for (var p = 0; p<game.config.CSSprefixes.length; p++)
					{
						...
						CSSstyleDeclaration += prefix+"transform:translate"+game.metrics.transforms3Dsupport[0]
						+"("
							+tile.newPosition.left.newValue.percent+"%,"
							+tile.oldPosition.top.percent+"%"
							+game.metrics.transforms3Dsupport[1]
						+"); "
				...
				case game.config.labels.DOWN:						
					...
					for (var p = 0; p<game.config.CSSprefixes.length; p++)
					{
						...
						CSSstyleDeclaration += prefix+"transform:translate"+game.metrics.transforms3Dsupport[0]
						+"("
							+tile.oldPosition.left.percent+"%,"
							+tile.newPosition.top.newValue.percent+"%"
							+game.metrics.transforms3Dsupport[1]
						+"); "
						+prefix+"transition:none; ";
					}					
			...
			if ( !game.config.pushMultiple || typeof tile.canPush.tiles == "undefined" ) { return; }
			...
			for (var i=0;i<tile.canPush.tiles.length;i++) {
				...
				switch(direction)
				{
					case game.config.labels.UP:
						...
						if (otherTileNewPos < tile.canPush.tiles[i].max || tile.canPush.tiles[i].max == null ) { tile.canPush.tiles[i].max = otherTileNewPos; } else { return false; }						
				...
				) { otherTile.revert = false; } else {  otherTile.revert = true; }
		...
		isTile: function (tile)	{
			...
			{
				case "object":
					if (tile.id.indexOf(game.config.labels.TILE_PREFIX)<0) { return false;}
				break;
		...
		moveTile: function(tile,params) {
			...
			var hole = game.runtime.tiles[game.config.labels.TILE_PREFIX+game.config.labels.HOLE_INDEX], temp = {};
		
			if ( !game.fn.canMove(tile)) {	return false; }					
			...
			if (params.refresh === true ) {
				game.fn.refresh({transitions:params.transitions,saveGame:!params.force,redraw:!!!params.force}); 
			}
		...
		shuffle: function(n,refresh) {	
			...
			if (typeof n == "undefined") { n = 10*game.config.size.n; }
			...
			{	
				...
				movableTiles = game.fn.getMovableTiles({direction:d,pushing:false,moving:true,force:true});
			...
			if (refresh) {	game.fn.refresh({transitions:true,transitionDuration:100}); }
			t1 = new Date(); 
		},
		...
		saveGame: function(type) {
			...
			if (!game.config.useLstorage) { return false; }
			if (game.runtime.ended != false) { return false; }			
			if (typeof type == "undefined") { type = "automatic"; }
			...
			if (game.runtime.started) { savedGame.startedDate = game.runtime.started.toUTCString(); }
		...
		loadGame: function(type) {
			if (!game.config.useLstorage) { return false; }
			if (typeof type == "undefined") { type = game.config.labels.AUTOSAVE_LABEL; }
			...
			if (loadedGames != null ) 
			{
				if (typeof loadedGames[type] != "undefined" && loadedGames[type] != null)
				{
					...
					game.fn.refresh({transitions:(type=="user")});
		...
		newGame: function() {
			...
			if (game.runtime.running)
			{
				...
				setTimeout(function(){ 
					...
					game.fn.loadGame(game.config.labels.SORTEDSAVE_LABEL);

					game.fn.refresh({transitions:false});
				...
				game.fn.animate({element:document.getElementById("game"),animation:"resetboard",duration:animationDuration,easing:"ease-in-out"});
			...
			else
			{
				...
				game.fn.animate({element:document.getElementById("game"),animation:"intro",duration:animationDuration});
				
			}
					
			setTimeout(function(){ 
			
				game.fn.shuffle(game.config.size.n*game.config.size.n);
				game.fn.saveGame(game.config.labels.SHUFFLEDSAVE_LABEL);
				game.fn.start();
		...
		moveAnyTile: function(params) {
			...
			var tiles = (params.pushing == true) ? game.fn.getMovableTiles({
													pushing:false,
													moving: true,
													direction: params.direction
											}).concat(game.fn.getMovableTiles({
													pushing:true,
													moving: false,
													direction: params.direction
											}).sort(function(t1, t2) { return t1.canPush.n - t2.canPush.n; }))
											:
											game.fn.getMovableTiles({
													pushing:false,
													moving: true,
													direction: params.direction
											});
			...
			{
				game.fn.moveTile(tiles[i],{transitions:false,force:false,refresh:false});				
			}
			game.fn.refresh({transitionDuration:game.config.transitionDuration.byKeyboard});
		...
		animate: function(params) {
			...
			if (typeof params		 		== "undefined"	||
				...
				typeof params.animation 	!= "string")	{ return false; }
			if (typeof params.duration 		!= "number") 	{ params.duration = game.config.transitionDuration.generic; }
			...
			for (var p = 0; p<game.config.CSSprefixes.length; p++)
			{
				...
				if (typeof params.iterations 	!= "undefined")	{ CSSstyleDeclaration += prefix+"animation-iteration-count:"+params.iteration+";"; }
				
				if (typeof params.delay 		== "number") 	{ CSSstyleDeclaration += prefix+"animation-delay:"+params.delay+"ms;";	}
				
				if (typeof params.easing 		== "string") 	{ CSSstyleDeclaration += prefix+"animation-timing-function:"+params.easing+";"; }
